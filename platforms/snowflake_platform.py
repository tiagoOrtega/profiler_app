"""Snowflake platform connector — wraps the existing SnowflakeConnection."""
...
class SnowflakePlatform(BasePlatform):
    dialect = "snowflake"

    def __init__(self, sf_config):
        self._sf = SnowflakeConnection(sf_config)

    # ── Execution ──────────────────────────────────────────────────────────────

    def fetch_one(self, sql: str, params=None):
        return self._sf.fetch_one(sql, params)

    def fetch_all(self, sql: str, params=None) -> list:
        return self._sf.fetch_all(sql, params)
        ...
        for i in range(n):
            for j in range(n):
                if i == j:
                    ...
                    qi, qj = f'"{col_names[i]}"', f'"{col_names[j]}"'
